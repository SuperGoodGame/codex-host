...
describe("parseClaudeGoalSignal", () => {
  ...
  it("recognises Stop hook verdicts and unrecoverable-error notices", () => {
    expect(
      parseClaudeGoalSignal({
        type: "user",
        message: {
    ...
    ).toEqual({ type: "verdict", condition, reason: "count is 1" });
    expect(
      parseClaudeGoalSignal({
        type: "system",
...
describe("classifyClaudeGoalCommandOutput", () => {
  it("maps every native acknowledgement", () => {
    ...
    expect(classifyClaudeGoalCommandOutput(`Goal active: ${condition} (2m)`)).toEqual({
      kind: "active",
      status: `${condition} (2m)`,
    });
    expect(
...
describe("ClaudeGoalTracker", () => {
  it("publishes only real transitions", () => {
    const emit = vi.fn();
    const tracker = new ClaudeGoalTracker(emit, () => 1_000);
    tracker.confirmCleared();
    expect(emit).not.toHaveBeenCalled();
    tracker.confirmSet(condition);
    expect(emit).toHaveBeenLastCalledWith({
      type: "session.goal.changed",
      goal: { objective: condition, setAtMs: 1_000, iterations: 0 },
    });
    tracker.apply({ type: "verdict", condition: "another goal", reason: "ignored" });
    expect(emit).toHaveBeenCalledTimes(1);
    tracker.apply({ type: "verdict", condition, reason: "count is 1" });
    expect(emit).toHaveBeenLastCalledWith({
      type: "session.goal.changed",
      goal: { objective: condition, setAtMs: 1_000, iterations: 1, lastReason: "count is 1" },
    });
    expect(
      tracker.reconcile({
        goal: { objective: condition, setAtMs: 5, iterations: 1, lastReason: "count is 1" },
      }),
    ).toBe(false);
    expect(emit).toHaveBeenCalledTimes(2);
    expect(tracker.reconcile({ goal: null, outcome: "unachievable", reason: "never" })).toBe(true);
    expect(emit).toHaveBeenLastCalledWith({
      type: "session.goal.changed",
      goal: null,
    ...
    tracker.apply({ type: "clearedByError", reason: "ignored without a goal" });
