...
export class ThreadGoalParamsError extends Error {}
...
export function parseThreadGoalSetParams(value: unknown): ThreadGoalSetParams {
  if (!isRecord(value) || typeof value.threadId !== "string" || value.threadId.length === 0) {
    throw new ThreadGoalParamsError("thread/goal/set params.threadId must be non-empty text");
  }
  const objective = value.objective ?? null;
  if (objective !== null && typeof objective !== "string") {
    throw new ThreadGoalParamsError("thread/goal/set params.objective must be text or null");
  }
  const status = value.status ?? null;
  if (status !== null && (typeof status !== "string" || !THREAD_GOAL_STATUSES.has(status))) {
    throw new ThreadGoalParamsError("thread/goal/set params.status is not a Goal status");
  }
  ...
  if (
    ...
    throw new ThreadGoalParamsError(
      "thread/goal/set params.tokenBudget must be a positive integer",
    );
  }
  return {
...
export function goalObservablyChanged(
  previous: ExternalThreadGoal | null,
  next: ExternalThreadGoal,
): boolean {
  return (
    previous === null ||
