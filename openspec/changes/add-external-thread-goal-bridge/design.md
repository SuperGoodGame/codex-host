...
- `/goal <cond>` 的本地输出是 `model: "<synthetic>"` 的 Assistant 消息 `Goal set: …`，随后模型在同一 query loop 内工作；Stop hook 的未达成判定以 User 消息 `Stop hook feedback:\n[<cond>]: <reason>` 出现；整个 Goal loop 只有一个 `result`。
...
- `Stop hook feedback` 解析依赖 CLI 文本；版本升级需重新核实（文档已固定版本）。
- `tokenBudget` 只回显不强制。
