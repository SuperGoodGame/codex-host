...
import {
  ThreadGoalParamsError,
  externalThreadGoalFromHarness,
  goalObservablyChanged,
...
/**
 * A Harness that emits every File Change from native patch evidence already
 * owns the File Change semantics, so generic projection must not infer a second
 * File Change from the same Tool input.
 */
...
export class AppServerHost {
  ...
  async #loadExternalGoal(thread: ExternalThread, control: HarnessGoalCapability): Promise<void> {
    ...
    if (result.value.goal && !thread.goal) {
      thread.goal = externalThreadGoalFromHarness(
        result.value.goal,
        null,
        thread.latestUsage,
        Date.now(),
    ...
    try {
      ...
      await this.#writer.json(
        rpcError(
          request,
          -32602,
          error instanceof ThreadGoalParamsError ? error.message : "Invalid thread/goal/set params",
        ),
      );
      return;
    }
  ...
  async #applyHarnessGoalChange(
    ...
    if (event.goal) {
      ...
      thread.goal = next;
      if (goalObservablyChanged(previous, next)) await this.#writeExternalGoalUpdated(thread);
      return;
    }
    if (!previous) return;
