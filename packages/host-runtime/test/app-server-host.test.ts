...
describe("AppServerHost External Thread Goals", () => {
  ...
  it("serves Codex Goal control from the Harness-owned Goal", async () => {
    ...
    // Native evaluation rounds prove the Harness is still pursuing the Goal.
    session.publishGoal({
      objective,
      setAtMs: Date.now(),
      iterations: 1,
      lastReason: "count is 1",
    });
    await waitForGoalUpdate(fixture, "active", 2);
  ...
  it("hydrates a Goal the Harness restored and rejects invalid updates", async () => {
    ...
    const session = adapter.sessions[0];
    if (!session) throw new Error("Fake Session was not opened");
    session.activeGoal = { objective, setAtMs: 1_000, iterations: 2, lastReason: "not yet" };
    ...
    ).resolves.toMatchObject({ result: { goal: { objective, status: "active", createdAt: 1 } } });
    writeRequest(fixture.desktopInput, {
      id: 81,
      method: "thread/goal/set",
