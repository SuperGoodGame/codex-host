...
export class FakeHarnessSession implements HarnessSession {
    ...
    if (supportsGoal) {
      this.goal = {
        ...
        read: async () => {
          ...
          return { ok: true, value: { goal: this.activeGoal } };
        },
      };
    }
    this.cwd = cwd;
  ...
  #setGoal(input: { turnId: HostTurnId; objective: string }): HarnessResult<TurnStartAccepted> {
    ...
    this.activeGoal = { objective: input.objective, setAtMs: Date.now(), iterations: 0 };
  ...
  #clearGoal(): HarnessResult<GoalClearCompleted> {
    if (this.#closed) return { ok: false, error: invalidStateError };
